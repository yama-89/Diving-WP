<!-- スライダーのコンテナを作成 -->
<div id="<?php echo esc_attr($args['post_type']); ?>" class="slider">
  <?php
  ...
  if ($the_query->have_posts()) : // 投稿がある場合
    while ($the_query->have_posts()) : $the_query->the_post(); // 投稿ループの開始
      ...
      //$image_spが配列であることを確認し、配列の場合、[0]番目の要素（画像のURL）を取得、そうでない場合、""とする
      $image_sp_src = is_array($image_sp) ? $image_sp[0] : "";
